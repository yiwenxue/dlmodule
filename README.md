...
To make some dynamically loadable, you will need a dynamic data structure at first. For the modules, I use doubly linked list written by myself [[Algorithm & Data Structure#doubly linked list]]. Queue and stack are not good choices.

## Dynamically loadable module
When you want to create a module, you must have a purpose, so you will need a **function** to make it. And at the same time, you will also need some data or information to tell your function where and when it should operate. 

When a module is loaded, the loader will process a initialization for a module, but how? The module may be created for different purpose, so their initialization must be different. One method is that: let the module define it's initialization routine.
### module in shared library
So the loader will just get a pointer to the initializer, and then call it. Yeah, call it as a callback function. And some necessary data/informarion should be send to this callback. But the data can't be in the same type for all modules, it should be defined by the creator according to it's requirements. So the loader will just pass a pointer (as void) to the data provided by the module to it's initializer.

Usually, the destroy function can be put into the same function. Let's name it as **load** or something. 
```C
...
So let's make a data struct for the module:
```C
struct module_data {
	const char 		*name;
	int 			(*load)(int , void *);
...
After we wrote a module, it's time to compile it. The technique here is dynamic loadable library, so we will compile this module as a share library. And that's why we don't have a main function in the source of  the module.
...
So we can't just push the `module_data` into a module list. There are much information for us to maintain until the end. Another data container is introduced for the main program. This data container includes all information of a module. For example: 
...
After that the module will be insert to the end/begining of the module.
Finished!
