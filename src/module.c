...
int modload(struct mod_list_h *mlist_hp, char *mod_name)
{
#if debug
...
int modunload(struct mod_list_h *mlist_hp, struct module *mod)
{
#if debug
...
int modappend(struct mod_list_h *mlist_hp, struct module *mod)
{
    LIST_INSERT_HEAD(mlist_hp, mod, m_list);
    return 0;
}

int modpop(struct mod_list_h *mlist_hp, struct module *mod)
{
    LIST_REMOVE(mod, mlist_hp, module, m_list);
    return 0;
}
...
struct module *modread(char *modulename)
{
    ...
    /* If any abnormal behavior happend, close the handle, and unload the
     * module. */
    if (dlerror() != NULL) {
        fprintf(stderr, "  [modread] Some errors appear on module %s\n",
                modulename);
...
int modctl_load(struct mod_list_h *mlist_hp, char *mod_name)
{
    return modload(mlist_hp, mod_name);
}

int modctl_unload(struct mod_list_h *mlist_hp, char *mod_name)
{
    struct module *var, *tvar;
    int erro = 0;
    LIST_FOREACH_SAFE(var, mlist_hp, m_list, tvar)
    {
        if (strncmp(var->moduledata->name, mod_name, MOD_NAME_LEN) == 0) {
            erro = modunload(mlist_hp, var);
            return erro;
        }
    }
    return -1;
